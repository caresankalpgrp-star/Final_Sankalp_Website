...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (req.method === 'POST') {
      const { name, location, text, rating, project, avatar_url, sort_order } = req.body;
      if (!name || !text) return res.status(400).json({ error: 'Name and text required' });
      const { data, error } = await supabase
        .from('testimonials')
        .insert({ name, location, text, rating: rating || 5, project, avatar_url: avatar_url || null, sort_order: sort_order || 0 })
        .select().single();
      if (error) throw error;
